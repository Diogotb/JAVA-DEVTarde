...
public class Serializacao {
    ...
    public static void serializarAgendamento(String arquivoAgendamento, List<Agendamento> agendamentos) {
        try (ObjectOutputStream outputStream = new ObjectOutputStream(new FileOutputStream(arquivoAgendamento))) {
            // Escreve a lista de usuários no arquivo especificado
            outputStream.writeObject(agendamentos);
            System.out.println("Dados do Agendamento serializados com sucesso.");
        } catch (IOException e) {
            // Em caso de erro na serialização, imprime a mensagem de erro
            System.err.println("Erro ao serializar os dados: " + e.getMessage());
        }
